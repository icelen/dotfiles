...
prompt_context() {
  ...
  if [[ "$user" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    prompt_segment black default "%(!.%{%F{yellow}%}.)$user@$(box_name)"
    # prompt_segment black default "%(!.%{%F{yellow}%}.)$user@%{$fg_bold[blue]%}%m%{$reset_color%}"
  fi
}
...
prompt_vimode() {
  prompt_segment black blue ${VIMODE}
}
...
prompt_ranger() {
  prompt_segment black red ${RANGERPROMPT}
}
## Main prompt
build_prompt() {
  RETVAL=$?
